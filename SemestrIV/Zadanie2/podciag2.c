...
void LCS_length_all(char *x, char *y, int c[SIZE_X+1][SIZE_Y+1], char b[SIZE_X][SIZE_Y]) {
  
  if (SIZE_X > SIZE_Y) {
    for (int i = 0; i <= SIZE_X-SIZE_Y; i++) {
      LCS_length(x+i, y, c, b);
      LCS_print(x+i, y, SIZE_X-1-i, SIZE_Y-1, b);
    }
  }
